...
class _SFormInSituSample extends State<SFormInSituSample> {
  ...
  Future<void> _openScanner() async {
    ...
    if (scannedCode != null && scannedCode is String) {
      setState(() => _barcode.text = scannedCode);
    }
  }
              ...
              Expanded(
                child: DropdownSearch<localUser>(
                  dropdownSearchDecoration: InputDecoration(
                    labelText: "2nd sampler",  // <-- This is your label
                    border: OutlineInputBorder(),
                  ),
                  items: sampler1,
            ...
            children: [
              Expanded(
                child: DropdownSearch<localState>(
                  dropdownSearchDecoration: InputDecoration(
                    labelText: "Select state",  // <-- This is your label
                    border: OutlineInputBorder(),
                  ),
                  items: state1,
                ...
                children: [
                  Expanded(
                    child: DropdownSearch<localDistinctLocation>(
                      dropdownSearchDecoration: InputDecoration(
                        labelText: "Select category",  // <-- This is your label
                        border: OutlineInputBorder(),
                      ),
                      items: category1,
                ...
                children: [
                  Expanded(
                    child: DropdownSearch<localLocation>(
                      dropdownSearchDecoration: InputDecoration(
                        labelText: "Select location",  // <-- This is your label
                        border: OutlineInputBorder(),
                      ),
                      items: location1,
            ...
            children: [
              ...
              Expanded(
                ...
                child: DropdownButtonFormField(
                  ...
                  validator: (value) =>
                  value == null ? 'Choose weather' : null,
                  onSaved: (value) => setState(() => _weather = value.toString()),
                ),
              ),
            ],
          ),
            ...
            children: [
              ...
              Expanded(
                ...
                child: DropdownButtonFormField(
                  ...
                  validator: (value) =>
                  value == null ? 'Choose tide level' : null,
                  onSaved: (value) => setState(() => _tide = value.toString()),
                ),
              ),
            ],
          ),
            ...
            children: [
              ...
              Expanded(
                ...
                child: DropdownButtonFormField(
                  ...
                  validator: (value) =>
                  value == null ? 'Choose sea condition' : null,
                  onSaved: (value) => setState(() => _tarball = value.toString()),
                ),
              ),
            ],
          ),
              ...
              child: Column(
                children: [
                  ...
                  DropdownButton<UsbDevice>(
                    ...
                    items: _devices.map((device) {
                      return DropdownMenuItem(
                        value: device,
                        child: Text("${device.manufacturerName} ${device.productName}"),
                      );
                    }).toList(),
                  ...
                  ElevatedButton(
                    ...
                    child: Text("Connect and Read YSI"),
                  ),
                  SizedBox(height: 20),
                  Text(_rawData,style: TextStyle(fontWeight: FontWeight.bold),),
                ],
              ),
            ) : SizedBox(width: 0.0,),
            /*
              ...
              Expanded(
                child: Column(
                  children: [
                    SizedBox(height: 8),
                    DropdownButton<UsbDevice>(
                      hint: Text("Select Sonde Device (Bluetooth)",style: TextStyle(fontSize: 12),),
                      value: _selectedDevice,
                      items: _devices.map((device) {
                        return DropdownMenuItem(
                          value: device,
                          child: Text("Test Bluetooth", style: TextStyle(fontSize: 12),),
                        );
                      }).toList(),
                      onChanged: (device) {
  ...
  Future<void> connectToDevice(UsbDevice device) async {
    ...
    setState(() {
      _uuidData.text = device.pid.toString();
    });
  ...
  void parseSensorData(String data) {
    ...
    for (var part in parts) {
      if (part.startsWith("pH:")) {
        _phData.text = part.replaceAll("pH:", "").trim();
      } else if (part.startsWith("DO:")) {
        _doData1.text = part.replaceAll("DO:", "").trim();
        _doData2.text = part.replaceAll("DO:", "").trim();
      } else if (part.startsWith("Temp:")) {
        _tempData.text = part.replaceAll("Temp:", "").trim();
      } else if (part.startsWith("SAL:")) {
        _sanilityData.text = part.replaceAll("SAL:", "").trim();
      } else if (part.startsWith("EC:")) {
        _ecData.text = part.replaceAll("EC:", "").trim();
      } else if (part.startsWith("TDS:")) {
        _tdsData.text = part.replaceAll("TDS:", "").trim();
      } else if (part.startsWith("TURB:")) {
        _turbidityData.text = part.replaceAll("TURB:", "").trim();
      } else if (part.startsWith("TSS:")) {
        _tdsData.text = part.replaceAll("TSS:", "").trim();
      } else if (part.startsWith("BATT:")) {
        _batteryData.text = part.replaceAll("BATT:", "").trim();
      }
    }
  ...
  Future<void> _connectToDevice() async {
    ...
    if (_selectedDevice == null) {
      ...
      // Send YSI command to get data (try "M\r" or "R\r")
      //String command = "M\r";
      //await _port!.write(Uint8List.fromList(command.codeUnits));

      // Wake Sonde and send command
      await _wakeYsiSonde(_port!);
    }

  }

  Future<void> _wakeYsiSonde(UsbPort port) async {
    print("Waking YSI Sonde...");
    // Send multiple carriage returns to wake up device
    await port.write(Uint8List.fromList("\r\r\r".codeUnits));
    await Future.delayed(Duration(milliseconds: 300));

    // Send "M" command (or "R" or "@000M!" depending on model)
    await port.write(Uint8List.fromList("M\r".codeUnits));
  }
...
class _ScannerScreenState extends State<ScannerScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: mobileScanner.MobileScanner(
